...
const API_KEY_CITY = "No3jClVpiGAsiXD5DACGMddvcu68aaLD";
...
function returnInnerHtml(
  ...
  return `<p class="curr-time">${format_time(hourlyArr[i].dt + offset)}</p>
  <div class="middle">
    <img src="http://openweathermap.org/img/wn/${
      hourlyArr[i].weather[0].icon
    }@2x.png" 
  ...
  <p class="value">${Math.floor(
    weatherProperty === "pop"
      ? hourlyArr[i][weatherProperty] * 100
      : hourlyArr[i][weatherProperty]
  )} ${unit}</p>
...
function putWeatherData(data) {
  ...
  if (swiperWrapper[0].children.length === 0) {
    createNodes(hourlyArr, "temperature-slider", "temp", "&deg;", offset);
    createNodes(hourlyArr, "humidity-slider", "humidity", "&percnt;", offset);
    createNodes(hourlyArr, "rainChance-slider", "pop", "&percnt;", offset);
  } else {
    updateNodes(hourlyArr, "temperature-slider", "temp", "&deg;", offset);
    updateNodes(hourlyArr, "humidity-slider", "humidity", "&percnt;", offset);
    updateNodes(hourlyArr, "rainChance-slider", "pop", "&percnt;", offset);
  }
